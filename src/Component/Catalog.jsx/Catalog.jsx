...
import { suit } from '../Collection/suit';
    ...
    const { addToCart } = useCart();
     ...
     // State for filter checkboxes
        ...
        const [outStock, setOutStock] = useState(false);
          ...
          const allproducts = [...jacket, ...shirt, ...suit]
                    ...
                    <div className="RightSide col-lg-9 col-md-12 col-sm-12 d-flex row">
                        {filteredProductsAll.map((product) => (
                            <div key={product.id} className="rightInner col-lg-3 col-sm-12 col-md-6 col-12 lh-lg pt-5">
                                <img height={400} style={{ width: "100%" }} src={product.image} alt={product.name} />
                                <h3>{product.name}</h3>
                                <del>{product.del}</del>
                                <span className="ps-2 fs-5 fw-bolder">{product.price}</span>
                                <button style={cartbtn} onClick={() => addToCart(product)}>Add To Cart</button>
                            </div>
                        ))}
                    </div>
                </div>
